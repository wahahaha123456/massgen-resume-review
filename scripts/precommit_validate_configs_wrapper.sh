#!/bin/bash
# Wrapper script to run config validation with uv in local environment
...
if [ -n "$CI" ] || [ -n "$GITHUB_ACTIONS" ]; then
    ...
    exec python3 "$(dirname "$0")/precommit_validate_configs.py" "$@"
else
    # Locally, use uv to ensure dependencies are available
    exec uv run python "$(dirname "$0")/precommit_validate_configs.py" "$@"
fi
